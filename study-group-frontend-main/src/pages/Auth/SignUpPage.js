...
function SignUpPage() {
  ...
  const [date, setDate] = useState(''); // ✅ 텍스트 입력을 받습니다.
  ...
  const handleSignUp = async (e) => {
    ...
    // 🚨 2. 필수 필드 유효성 검사 추가 (DB의 NOT NULL 제약 조건 보호)
    if (!name.trim()) {
        alert('이름을 입력해주세요.');
        return;
    }
    ...
    if (!gender.trim()) {
     ...
     try {
      ...
      const response = await api.post('/api/signup', {
        ...
        password,
        name,
        date, // "YYYY-MM-DD" 형식의 텍스트가 전송됩니다.
      ...
      if (error.response?.status === 409) {
        ...
        // DB NOT NULL 오류를 포함한 서버 오류 발생
        alert('❌ 서버 오류 발생 또는 필수 정보 누락');
      }
    }
  };
